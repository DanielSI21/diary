...
export default function PendingBanner({ day, notes, onChanged }: Props) {
  ...
  return (
    <div className="space-y-2 rounded-xl border border-amber-200 bg-amber-50 p-3 dark:border-amber-500/30 dark:bg-amber-500/10">
      {notes.map((n) => (
        <div key={n.id} className="space-y-1.5 text-sm">
          <p className="break-words">
            <span className="font-semibold text-amber-700 dark:text-amber-400">Pendiente: </span>
            <LinkedText text={n.text} className="text-slate-700 dark:text-slate-200" />
          </p>
